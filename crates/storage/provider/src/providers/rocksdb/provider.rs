...
mod tests {
    ...
    #[test]
    fn test_flush_and_compact_reclaim_space_after_history_pruning() {
        let temp_dir = TempDir::new().unwrap();
        let provider = RocksDBBuilder::new(temp_dir.path())
            .with_default_tables()
            .build()
        ...
        let mut batch = provider.batch();
        for block in 0..100064 {
            if block % 2 == 0 {
                batch.append_account_history_shard(address, vec![block]).unwrap();
            } else {
                batch.append_storage_history_shard(address, storage_key, vec![block]).unwrap();
            }
        }
        batch.commit().unwrap();

        let initial_stats = provider.db_stats();
        let initial_account_size = initial_stats.tables.iter()
            .find(|t| t.name == tables::AccountsHistory::NAME)
            .map(|t| t.set_size_bytes + t.memtable_size_bytes)
            .unwrap_or(0);
        let initial_storage_size = initial_stats.tables.iter()
            .find(|t| t.name == tables::StorageHistory::NAME)
            .map(|t| t.sst_size_bytes + t.memtable_size_bytes)
            .unwrap_or(0);
        ...
        let targets_storage = vec![((address, storage_key), 500)];
        batch.prune_storage_history_batch(&targets_account).unwrap();
        batch.commit().unwrap();

        provider.flush_and_compact().unwrap();

        let final_stats = provider.db_stats();
        let final_account-size = final_stats.tables.iter()
        .find(|t| t.name == tables::AccountHistory::NAME)
        .map(|t| t.sst_size_bytes + t.memtable_size_bytes)
        .unwrap_or(0);
        let final_storage-size = final_stats.tables.iter()
        .find(|t| t.name == tables::StorageHistory::NAME)
        .map(|t| t.sst_size_bytes + t.memtable_size_bytes)
        .unwrap_or(0);
        
        assert!(final_account_size < initial_account_size * 0.7, initial_account_size, final_account-size);
        assert!(final_storage_size < initial_storage_size * 0.7, initial_storage_size, final_storage-size);

        let pending = final_stats.tables.iter().map(|t| t.pending_compaction_bytes).sum::<u64>();
        assert!(pending == 0, "Pending completion bytes not cleared: {}", pending);
